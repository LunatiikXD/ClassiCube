...
static void LInput_Select(void* widget, cc_bool wasSelected) {
	struct LInput* input = (struct LInput*)widget;
	caretStart = DateTime_CurrentUTC_MS();
	LInput_MoveCaretToCursor(input);

	/* TODO: Only draw outer border */
	if (wasSelected) return;
	LWidget_Draw(widget);
	Window_OpenKeyboard(&input->text, input->password ? 
						KEYBOARD_TYPE_PASSWORD : KEYBOARD_TYPE_TEXT);
}
...
static void LInput_TextChanged(void* widget, const String* str) {
	struct LInput* w = (struct LInput*)widget;
